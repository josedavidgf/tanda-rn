...
import type { Schedule, CalendarEntry } from '@/types/calendar';
...
export default function CalendarScreen() {
    ...
    const { getMyShifts, removeShift } = useShiftApi();
    ...
    const { isWorker, loading, ready } = useIsWorkerReady();
    ...
    async function togglePreference(dateStr: string, shiftType: string) {
        ...
        try {
            if (alreadyExists) {
                ...
                console.log('No existe, creando preferencia');
                console.log('isWorker', isWorker.worker_id);
                console.log('dateStr', dateStr);
                console.log('preference_type', shiftType);
                console.log('hospital_id', isWorker.workers_hospitals?.[0]?.hospital_id);
                console.log('speciality_id', isWorker.workers_specialities?.[0]?.speciality_id);
    ...
    async function handleDeletePublication(shiftId: string, dateStr: string) {
        ...
        try {
            ...
            if (success) {
                const updatedEntry = { ...calendarMap[dateStr] };
                            ...
                            <Button
                                ...
                                onPress={async () => {
                                    const updates = Object.entries(draftShiftMap).map(([dateStr, entry]) =>
                                        entry.shift_type
                                            ? setShiftForDay(isWorker.worker_id, dateStr, entry.shift_type)
                                            : removeShiftForDay(isWorker.worker_id, dateStr)
                                    );
