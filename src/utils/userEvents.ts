...
export const USER_EVENT_CONFIG: Record<string, UserEventConfig> = {
  ...
  swap_proposed: {
    ...
    getDescription: (metadata) =>
      `Has ofrecido a ${metadata.shift_owner_name} ${metadata.shift_owner_surname} tu turno del ${formatFriendlyDate(metadata.offered_date)} de ${shiftTypeLabels[metadata.offered_type]} por su turno del ${formatFriendlyDate(metadata.shift_date)} de ${shiftTypeLabels[metadata.shift_type]}.`,
  },
  swap_received: {
    icon: Repeat,
    title: 'Te han propuesto un intercambio',
    getDescription: (metadata) =>
      `${metadata.requester_name} ${metadata.requester_surname} te ha ofrecido su turno del ${formatFriendlyDate(metadata.offered_date)} de ${shiftTypeLabels[metadata.offered_type]} por tu turno del ${formatFriendlyDate(metadata.shift_date)} de ${shiftTypeLabels[metadata.shift_type]}.`,
  },
  swap_accepted: {
    icon: CheckCircle,
    title: 'Tu intercambio ha sido aceptado',
    getDescription: (metadata) =>
      `Tu turno ofrecido del ${formatFriendlyDate(metadata.offered_date)} de ${shiftTypeLabels[metadata.offered_type]} ha sido aceptado. Recibirás el turno del ${formatFriendlyDate(metadata.shift_date)} de ${shiftTypeLabels[metadata.shift_type]}.`,
  },
  swap_rejected: {
    icon: XCircle,
